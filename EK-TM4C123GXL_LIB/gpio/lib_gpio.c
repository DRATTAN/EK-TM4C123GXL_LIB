...
void LIB_GPIO_ExtiInit(uint8_t GPIOISRPort, uint8_t GPIOISRPin, uint8_t Trigger, void (*ISR_handler)(void))
{
    ...
    else if(Trigger == GPIO_EXTI_TRIGGER_FALLING || Trigger == GPIO_EXTI_TRIGGER_LOW || Trigger == GPIO_EXTI_TRIGGER_BOTHEDGES) LIB_GPIO_Init(GPIO_BASE_TABLE[GPIOISRPort], 0x01 << GPIOISRPin, GPIO_DIR_INPUT, GPIO_PIN_TYPE_INPUT_WPD, 0x00);
